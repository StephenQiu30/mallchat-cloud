...
public class ChatRoomJoinApplyController {
    ...
    public BaseResponse<Long> applyJoinRoom(@Validated @RequestBody ChatRoomJoinApplyRequest request) {
        ThrowUtils.throwIf(request == null || request.getRoomId() == null, ErrorCode.PARAMS_ERROR);
        Long userId = SecurityUtils.getLoginUserId();
        return ResultUtils.success(chatRoomJoinApplyService.applyJoinRoom(request, userId));
    }

    @PostMapping("/approve")
    @OperationLog(module = "入群申请", action = "审核入群申请")
    @Operation(summary = "审核入群申请", description = "群主或管理员审核入群申请")
    public BaseResponse<Boolean> approveJoinRoom(@Validated @RequestBody ChatRoomJoinApproveRequest request) {
        ThrowUtils.throwIf(request == null || request.getApplyId() == null || request.getStatus() == null,
                ErrorCode.PARAMS_ERROR);
        Long userId = SecurityUtils.getLoginUserId();
        return ResultUtils.success(chatRoomJoinApplyService.approveJoinRoom(request, userId));
    }
