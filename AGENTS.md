...
1. 实施 IM 后端增强前，必须先检查对应 PRD、Design 文档、现有代码事实和测试覆盖，确认需求不是凭空新增，也没有绕过当前 `chat-*` 领域模型。
2. 每个功能点必须保持单一边界：好友、群聊、消息、会话、动态、通知、WebSocket 运行契约不得混成一个不可验收的大改动。
3. 执行顺序应优先处理基础契约和可靠性：先对齐 WebSocket 运行契约与实时投递可靠性，再做好友/群聊/富消息/已读回执，最后再扩展动态 feed 和通知中心聚合。
4. 动态 feed 不得复用 `chat_message` 作为事实表；通知中心不得替代好友申请、群申请、动态评论等业务事实表。
5. Redis 缓存只能作为加速层；好友关系、房间成员、消息、会话、通知、动态等事实必须以数据库或明确的持久化模型为准。
6. 涉及房间成员、好友关系、消息发送、已读、撤回、通知推送和 WebSocket 连接的改动，必须显式列出权限边界、幂等边界、并发边界和失败补偿边界。
7. 如果 PRD 中存在“待确认问题”，不得在实现中静默做最终选择；必须先在 Design 文档或任务说明中写明取舍，必要时向用户确认。
8. 每次进入实现前，需要补一段自审结论：本次做什么、不做什么、依赖哪些已有模块、哪些测试会先失败、哪些接口需要高可用验证。

## IM 生产化 Issue 消费规范

1. 生产化任务先进入长期 PRD/Design/Acceptance 文档或 GitHub Issue 验收说明，再创建或消费 GitHub Issue；不得先写代码后补规格，不为一次性执行过程新增 `docs/superpowers/` 或 `docs/plans/` 中间产物。
2. 首批 GitHub Issue 只创建 P0，范围限定为安全、消息可靠性、可观测性和数据恢复；P1/P2 只作为候选池。
3. 后端功能必须遵循 TDD：先写失败测试并确认失败，再写最小实现，再回归相关模块测试和 `bash scripts/validate-repository.sh`。
4. 每个子 Issue 必须声明 Parent Epic、文件所有权、TDD 验收、生产验收和完成标准。
5. 每轮并行最多消费 2-3 个子 Issue；多个子智能体不得同时写同一个模块核心文件或同一个长期设计文档。
6. 安全、权限、事务、消息事实和数据恢复类 Issue 必须安排测试验证人或只读 reviewer 复核。
7. 主智能体负责汇总子智能体结果、解决冲突、运行回归、更新验收文档或 Issue 结果、提交和推送。
8. Issue 完成状态不能替代验收状态；完成代码后必须同步验收文档和 GitHub Issue 结果。
9. 生产化 Issue 必须通过 PR 消费，不直接提交到 `main`；PR 编号从 `m1` 开始递增，分支使用 `m1-<issue-slug>`、`m2-<issue-slug>` 形式，PR 标题使用 `[m1] ...`、`[m2] ...` 形式。
10. m 系列 PR 默认只消费一个明确子 Issue；当用户明确要求按 Epic 聚合提交时，可以在同一 m PR 中消费同一 Parent Epic 下的多个子 Issue，但 PR 描述必须逐项列出关联 Issue、RED 测试证据、GREEN 验证命令、影响范围和残余风险。
...
3. CI 主路径优先运行仓库结构检查、工程化契约守护、核心 focused tests、编译和 Docker Compose 配置校验。
...
2. IM 核心接口必须优先覆盖高可用边界：消息重复发送、MQ 推送失败或重复、Redis 冷缓存、房间成员缓存缺失、WebSocket 断线重连、旧已读边界、撤回超时、好友关系变化后的权限收敛。
3. 每个后端任务的验收命令必须包含对应模块测试，例如 `mallchat-chat-service`、`mallchat-notification-service`、`mallchat-file-service`、`mallchat-common-websocket` 或网关测试；不能只运行编译命令替代行为测试。
4. 涉及数据库写入的接口应验证事务边界和幂等行为，尤其是好友申请、私聊房间创建、群成员加入、消息发送、通知创建和动态互动。
5. 涉及实时推送的接口应验证“落库成功后再推送”和“推送失败不破坏事实数据”两类行为。
6. 涉及缓存的接口应至少覆盖缓存命中和缓存缺失回源；不允许只验证缓存命中路径。
7. 涉及权限的接口应至少覆盖本人、好友、群成员、非成员、管理员或群主等不同角色边界。
8. 交付说明必须写清楚红灯测试、绿灯实现、最终测试命令、结果和仍未覆盖的高可用风险。
...
8. 执行 todo、临时任务清单、过程性进展记录、一次性排查记录不应写入 `docs/`；这类内容应保留在 Issue、PR 或 Workpad 中闭环。
...
9. CI 必须包含清晰的后端质量入口，至少运行 `bash scripts/validate-repository.sh`、工程化契约守护测试、核心 Maven focused tests、`mvn -B -DskipTests compile` 和必要的配置校验；新增稳定单元、集成、UI、快照或性能测试后，应接入对应 focused tests 或 CI 明确步骤。
