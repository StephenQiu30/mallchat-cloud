...
1. 实施 IM 后端增强前，必须先检查对应 PRD、OpenSpec spec、现有代码事实和测试覆盖，确认需求不是凭空新增，也没有绕过当前 `chat-*` 领域模型。
...
10. m 系列 PR 默认只消费一个明确子 Issue；当用户明确要求按 Epic 聚合提交时，可以在同一 m PR 中消费同一 Parent Epic 下的多个子 Issue，但 PR 描述必须逐项列出关联 Issue、OpenSpec change id、RED 测试证据、GREEN 验证命令、影响范围和残余风险。
...
3. 每个 OpenSpec change 的验收命令必须包含对应模块测试，例如 `mallchat-chat-service`、`mallchat-notification-service`、`mallchat-file-service`、`mallchat-common-websocket` 或网关测试；不能只运行编译命令替代行为测试。
