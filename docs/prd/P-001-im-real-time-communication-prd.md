...
1. 以 QQ-like 社交通讯为方向，首版形成“好友关系 -> 会话列表 -> 聊天详情 -> 实时消息 -> 未读/已读反馈 -> 好友动态”的闭环。
...
- 消息编辑、语音通话、视频通话、复杂空间装扮列为后续增强，不阻塞 MVP。
...
6. 动态 feed 最小闭环：文字/图片发布、好友可见列表、删除自己的动态、点赞、评论和互动通知。
...
3. 群公告、入群审核和群通知增强。
4. 管理端消息/群聊治理。
5. 动态可见性细化、评论回复、互动聚合和管理端治理。

### P2

1. 语音消息、视频消息、音视频通话。
2. 空间装扮、访客记录、相册体系和公开广场。
3. 多端离线缓存与漫游策略优化。
...
| 日期 | 作者 | 版本 | 变更说明 |
| --- | --- | --- | --- |
| 2026-05-20 | StephenQiu30 | 0.2.0 | 将基础动态 feed 提升为 QQ-like IM MVP 必选能力 |
